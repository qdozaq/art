...
public class sort_alpha2 extends PApplet {
...
public void doit(){
  ...
  for(int i = 0; i<colors.length; i++){
    colors[i] = finalImg.pixels[nonAlpha.get(i)];
  }

  // quicksort(colors, 0, colors.length-1);
  alphaSort(finalImg.pixels, nonAlpha);
...
public void alphaSort(int pixs[], ArrayList<Integer> alphas){
  ...
  for(int i=0; i<alphas.size(); i++){
      int r = round(random(150, 300));
      int offset = alphas.get(i) - round(random(200));
...
public boolean sortby(int a, int b){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sort_alpha2" };
    if (passedArgs != null) {
